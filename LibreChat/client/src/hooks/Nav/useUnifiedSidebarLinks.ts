...
import { BarChart3, MessagesSquare, FolderTree } from 'lucide-react';
...
import ProjectExplorerPanel from '~/components/SidePanel/ProjectExplorer/ProjectExplorerPanel';
...
export default function useUnifiedSidebarLinks() {
  ...
  const links = useMemo(() => {
    ...
    const projectExplorerLink: NavLink = {
    ...
    if (!insightsFeatureEnabled || insightsAccess?.access !== true) {
      return [conversationLink, projectExplorerLink, ...sideNavLinks];
    }
    ...
    return [conversationLink, projectExplorerLink, ...nextLinks];
